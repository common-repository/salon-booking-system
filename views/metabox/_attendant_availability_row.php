                        ...
                        <select class="sln-select select2-hidden-accessible" data-palceholder="<?php esc_html_e('Select a resource', 'salon-booking-system')?>"
                            ...
                            foreach($plugin->getRepository(SLN_Plugin::POST_TYPE_RESOURCE)->getAll() as $resource): ?>
                                <option class="red" value="<?php echo esc_attr($resource->getId())?>"
                                    <?php echo (isset($row['day_specific_resource']) && $row['day_specific_resource'] == $resource->getId()) ? 'selected=selected' : ''; ?>
                                    ><?php echo esc_attr($resource->getTitle()); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <select id="<?php echo SLN_Form::makeID($prefix. '[day_specific_service]'); ?>"
                            ...
                            foreach($services as $service):
                                ...
                                <option class="red" value="<?php echo esc_attr($service->getId())?>"
                                    data-resources="<?php foreach($resources as $resource){echo esc_attr($resource->getId()).','; $has_resource |= isset($row['day_specific_resource']) && $row['day_specific_resource'] == $resource->getId();} ?>"
                                    <?php echo !$has_resource && isset($row['day_specific_resource']) ? 'disabled="disabled"' : ''; ?>
                                    <?php echo (isset($row['day_specific_service']) && $row['day_specific_service'] == $service->getId()) ? 'selected=selected' : ''; ?>
                                    ><?php echo esc_attr($service->getName()); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
