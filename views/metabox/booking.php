...
<div class="sln-bootstrap" id="detailsWrapper">
    <?php
...
<div class="row">
    <div class="col-xs-12 sln-row">
        <h4 class="sln-box-title--nu--sec">
            <?php esc_html_e('Customer details', 'salon-booking-system')?>
        <?php if (preg_match('/post\-new\.php/i', $_SERVER['REQUEST_URI'])): ?>
            <span><a target="_blanck" class="sln-customer-url--icon sln-icon--adres-cart sln-btn--icon hide"></a></span>
        <?php endif; ?>
        </h4>
    </div>
        <div class="col-xs-12 col-sm-6 sln-select">
            <select id="sln-update-user-field"
                 ...
                 data-placeholder="<?php esc_html_e('Start typing the name or email or phone number', 'salon-booking-system')?>"
                 class="form-control">
            </select>
        </div>
            ...
            <div class="col-xs-12 col-sm-2">
                <div class="sln-booking-customer-score">
                    <div class="sln-booking-customer-score--title">
                        <?php esc_html_e('Score', 'salon-booking-system') ?>
                    </div>
            ...
            <div class="row">
                <div class="col-xs-12 col-md-6">
                    <div class="form-group sln_meta_field sln-select">
                        <label><?php esc_html_e('Status', 'salon-booking-system');?></label>
                        <?php
                ...
                <div class="col-xs-12 col-md-6 sln-set-default-booking-status--block-labels <?php echo !defined("SLN_VERSION_PAY")  ? 'sln-set-default-booking-status--block-label-disabled' : '' ?>" data-default-status="<?php echo SLN_Plugin::getInstance()->getSettings()->getDefaultBookingStatus() ?>">
    ...
    <div class="sln_booking-topbuttons sln-box sln-box--main">
        <div class="row">
            <div class="col-xs-12 col-lg-5 col-md-5 col-sm-6 sln_accept-refuse">
                <h2><?php esc_html_e('This booking waits for confirmation!', 'salon-booking-system')?></h2>
                ...
                <div class="row">
                    <div class="col-xs-12 col-lg-5 col-md-6 col-sm-6">
                        <button id="booking-accept" class="btn btn-success"
                            ...
                            <?php esc_html_e('Accept', 'salon-booking-system')?></button>
                    </div>
                    <div class="col-xs-12 col-lg-5 col-md-6 col-sm-6">
                        <button id="booking-refuse" class="btn btn-danger"
                                data-status="<?php echo SLN_Enum_BookingStatus::CANCELED ?>">
                            <?php esc_html_e('Refuse', 'salon-booking-system')?></button>
                    </div>
                </div>
            </div>
        </div>
...
<div class="sln-box__collapsewrp <?php echo in_array(SLN_Plugin::USER_ROLE_WORKER,  wp_get_current_user()->roles) ? 'sln-disabled' : '' ?>" id="collapseMoreDetailsWrapper">
    ...
    <div class="sln-box__collapse collapse" id="collapseMoreDetails">
        <div class="sln-box sln-box--main">
            <div class="sln-box__fl">
                ...
                <div class="sln-box__fl__item sln-box__fl__item--transaction sln-input--simple">
                    <?php echo $booking->getTransactionId() ?
                        '<h5 class="sln-box-title--nu--ter sln-box-title--nu--dark">' . esc_html__("Transaction", 'salon-booking-system') . ': <strong>' . esc_attr(implode(', ', $booking->getTransactionId())) . '</strong></h5>' :
                        '<h5 class="sln-box-title--nu--ter sln-box-title--nu--dark">' . esc_html__("Transaction", 'salon-booking-system') . ': <strong>' . esc_html__('n.a.', 'salon-booking-system') . '</strong></h5>';
                    ?>
                </div>
            ...
            <div class="sln-box__fl__item sln-box__fl__item--full">
                <button class="sln-btn sln-btn--main sln-btn--big sln-btn--icon sln-icon--settings" id="calculate-total"><?php esc_html_e('Calculate total', 'salon-booking-system')?></button>
...
<?php if (class_exists('\SalonSOAP\Addon')) {
    echo $plugin->loadView('metabox/soap_notes', array('postType' => $postType, 'booking' => $booking, 'helper' => $helper));
} ?>
