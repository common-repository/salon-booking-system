...
class ServicesCategories_Controller extends REST_Controller
{
    ...
    public function prepare_response_for_collection($category)
    {
        return array(
            ...
            'image' => wp_get_attachment_image_url(get_term_meta($category->term_id, self::TAXONOMY_SLUG. '_image', true))
        );
    }
    ...
    protected function save_item_term($request, $id = 0)
    {
        ...
        if ( is_wp_error($result) ) {
            throw new \Exception(esc_html__( 'Save category error.', 'salon-booking-system' ));
        }
